...
public abstract class Map {
	...
	public static void refresh(){
		...
		if(center == null || playerPos.getGx() != center.getCoord()[0] || playerPos.getGy() != center.getCoord()[1] || playerPos.getGz() != center.getCoord()[2]) {
	    	refreshMap();
			
			HashSet<Entity> npcs = new HashSet<>();
			for (int lx = 0; lx < mapInChunks.length; lx++){
				for(int ly = 0; ly < mapInChunks[0].length; ly++){
					npcs.addAll(mapInChunks[lx][ly].getEntities(e -> e.has(TimedComponent.class)));
				}
			}
			npcs.add(Main.PLAYER);
